  ...
  .then((data) => {
    ...
    Actor.create(actors, function (err, actors) {
      if (err) {
        ...
        const managers = actors.filter((actor) => actor.role.includes('MANAGER'));
        const explorers = actors.filter((actor) => actor.role.includes('EXPLORER'));
        for (let i = 0; i < searchCriterias.length; i++) {
          searchCriterias[i].explorer = explorers[Math.floor(Math.random() * (explorers.length - 1))]._id.valueOf();
          const newSearchCriteria = new SearchCriteriaModel(searchCriterias[i]);
        ...
        for (let i = 0; i < trips.length; i++) {
          trips[i].manager = managers[Math.floor(Math.random() * (managers.length - 1))]._id.valueOf();
          ...
          newTrip.save(function (err, trip) {
            ...
            if (!err && i === trips.length - 1) {
              for (let j = 0; j < applications.length; j++) {
                applications[j].trip = trips[Math.floor(Math.random() * (trips.length - 1))]._id.valueOf();
